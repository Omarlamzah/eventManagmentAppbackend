...
if (isset($_GET['search_user'])) {
    ...
    if(!empty($query)){
        ...
        $total_users = count($userController->getAllUsersPaginationcount( $query));
        echo   $total_users ."ueses countnnn";
    }
    else {
...
.pagination-link {
    text-decoration: none;
    color: blue; /* Change this to your desired link color */
}

.pagination-link.active {
    font-weight: bold;
    color: red; /* Change this to your desired active link color */
}
...
<form method="post">
    ...
    <h2 class="user-count container">All Users in this page <?php echo count($users); ?></h2>
        ...
        <?php foreach ($users as $user) : ?>
            <tr class="<?php if($user['Etat']=="oui") echo "is-selected";?>" >
                ...
                <td style="display: flex; gap:4px; padding:10px;">
                     <!--vérifier modal -->
                     <button class="button is-info is-dark" onclick="CustomAlertcheck('file status',  '<?php echo $user['Fichier']; ?>')">vérifier 
                     
                     <img width="30px" src="./public/asset/icon/<?php if($user['Fichier']){echo "check.png";} else { echo "uncheck.png";} ?>" alt="" srcset="">
                    </button>                
                    <!--Joindre modal -->
                   <!--joinder  with alertify -->
                   <button  class='button is-success'onclick="CustomAlert('choose files', `<form onsubmit='uploadFile(event)' action='' method='post' enctype='multipart/form-data'>
    <div class='field'>
        ...
        <div class='control'>
            <button type='submit' class='button is-success'>Upload</button>
...
</form>`);">joinder <img width="30px" src="./public/asset/icon/insert.png" alt="" srcset=""></button>
                   ...
                   <!--joinder  with alertify -->
                    ...
                    <!--update modal -->
                    <p><a href="#showusermodal<?php echo  $user['ID']?>" class="button is-info" rel="modal:open">Modife <img width="30px" src="./public/asset/icon/edit.png" alt="" srcset=""></a></p>
...
function CustomAlertcheck(title,filename){
    
    if(filename){
        const msg="file exist"+` <a download href="./uploads/presentation/${filename}"> dounwload ${filename}</a>`
